...
.paweh_tdt_values <- function(input) {
  shiny::reactiveValuesToList(input)
  defaults <- .paweh_tdt_defaults()
...
.paweh_tdt_num <- function(x, label, lower, upper, open = FALSE) {
...
.paweh_tdt_ui <- function(id) {
  ns <- shiny::NS(id)
  shiny::div(
    class = "paweh-workspace",
    .paweh_page_heading(
        ...
        bslib::nav_panel(
          ...
          shiny::tags$details(
            ...
            shiny::div(
              ...
              shiny::p(class = "paweh-advanced-subtitle", "Explore two assumptions simultaneously using the frozen model-based design."),
...
.paweh_tdt_snapshot <- function(values) {
  ...
  if (values$objective == "power") {
    ...
    .paweh_tdt_num(values$target_power, "Target power", 0, 100, TRUE)
  ...
  if (values$input_mode == "model_based") {
    .paweh_tdt_num(values$prev, "Disease prevalence", 0, 100, TRUE)
    .paweh_tdt_num(values$pd, "Modeled-allele frequency", 0, 100, TRUE)
    .paweh_tdt_num(values$R1, "Heterozygote relative risk", 0, Inf, TRUE)
    .paweh_tdt_num(values$R2, "Homozygote relative risk", 0, Inf, TRUE)
    .paweh_tdt_num(values$delta_prime, "D\u2032", 0, 1)
    args <- c(args, list(
    ...
    .paweh_tdt_num(values$ET, "Expected transmitted count", 0, Inf)
    .paweh_tdt_num(values$ENT, "Expected non-transmitted count", 0, Inf)
    ...
    if (values$objective == "mssn") {
      .paweh_tdt_num(values$n_trios, "Represented affected-child trios", 0, Inf, TRUE)
  ...
  .paweh_tdt_num(values$misclass_rate, "Phenotype misclassification rate", 0, 100)
  .paweh_tdt_num(values$heter_rate, "Locus heterogeneity rate", 0, 100)
...
.paweh_tdt_call <- function(snapshot, args = snapshot$backend_args) {
  args$verbose <- FALSE
...
.paweh_tdt_calculate <- function(snapshot) {
  result <- .paweh_tdt_call(snapshot)
  list(
...
.paweh_tdt_sig <- function(values) serialize(values, NULL)
.paweh_tdt_pct <- function(x, digits = 1) {
  if (!is.finite(x)) return("not defined")
  paste0(formatC(100 * x, format = "f", digits = digits), "%")
}
.paweh_tdt_rate <- function(x, digits = 1) paste0(formatC(100 * x, format = "f", digits = digits), "%")
.paweh_tdt_count <- function(x, plan = FALSE) {
...
.paweh_tdt_repro_args <- function(calculation) {
  s <- calculation$snapshot
...
.paweh_tdt_repro_call <- function(calculation) {
  .paweh_repro_call(
    if (calculation$snapshot$objective == "power") "tdt_power" else "tdt_mssn",
    .paweh_tdt_repro_args(calculation)
  )
}
.paweh_tdt_scenario_detail_rows <- function(calculation, scenario) {
  result <- calculation$result
  rows <- list(
    .paweh_summary_row("Expected transmitted probability (gT*)", formatC(result$gT_star[[scenario]], format = "f", digits = 4)),
    .paweh_summary_row("Expected non-transmitted probability (gNT*)", formatC(result$gNT_star[[scenario]], format = "f", digits = 4))
  )
  if (calculation$snapshot$objective == "power") rows <- c(rows, list(
    .paweh_summary_row("Expected transmitted count", formatC(result$ET[[scenario]], format = "f", digits = 2)),
    .paweh_summary_row("Expected non-transmitted count", formatC(result$ENT[[scenario]], format = "f", digits = 2)),
    .paweh_summary_row("Non-centrality parameter", formatC(result$lambda[[scenario]], format = "f", digits = 4)),
    .paweh_summary_row("Power", .paweh_tdt_pct(result$power[[scenario]], 2))
  )) else {
    rows <- c(rows, list(
      .paweh_summary_row("Required affected-child trios", .paweh_tdt_count(result$N[[scenario]], plan = TRUE)),
      if (scenario != "no_error") .paweh_summary_row(
      ...
      .paweh_summary_row("Power at baseline trio count", .paweh_tdt_pct(result$power_at_N_no_error[[scenario]], 2))
    ))
  }
  rows
}
.paweh_tdt_advanced_ui <- function(calculation) {
  s <- calculation$snapshot
  result <- calculation$result
  model_rows <- if (s$input_mode == "model_based") list(
    .paweh_summary_row("Disease prevalence", formatC(result$model_parameters$prev, format = "f", digits = 4)),
    .paweh_summary_row("Modeled-allele frequency", formatC(result$model_parameters$pd, format = "f", digits = 4)),
    .paweh_summary_row("R1", formatC(result$model_parameters$R1, format = "f", digits = 4)),
    .paweh_summary_row("R2", formatC(result$model_parameters$R2, format = "f", digits = 4)),
    .paweh_summary_row("D\u2032", formatC(result$model_parameters$delta_prime, format = "f", digits = 4)),
    .paweh_summary_row("Alpha", format(result$alpha))
  ) else list(
    .paweh_summary_row("Input specification", "Direct transmission quantities"),
    .paweh_summary_row("ET", format(s$backend_args$ET)),
    .paweh_summary_row("ENT", format(s$backend_args$ENT)),
    if (s$objective == "mssn") .paweh_summary_row("Represented affected-child trios", format(s$backend_args$n_trios)),
    .paweh_summary_row("Alpha", format(result$alpha))
  )
  scenarios <- .paweh_tdt_scenarios(calculation)
  .paweh_advanced_details_ui(
    ...
    lapply(scenarios, function(scenario) {
      .paweh_detail_section(
        paste(unname(.paweh_tdt_scenario_labels[scenario]), "transmission quantities"),
        .paweh_tdt_scenario_detail_rows(calculation, scenario)
      )
    }),
    if (calculation$active$misclassification) .paweh_detail_section(
      "Phenotype misclassification details",
      list(.paweh_summary_row("Control-to-affected rate", .paweh_tdt_rate(result$model_parameters$misclass_rate)))
    ),
    if (calculation$active$heterogeneity) .paweh_detail_section(
    ...
    .paweh_reproduce_ui(.paweh_tdt_repro_call(calculation))
  )
}

.paweh_tdt_result_card <- function(calculation, scenario) {
  ...
  if (snapshot$objective == "power") {
    body <- shiny::tagList(
      rate_row,
      .paweh_summary_row("Expected power", .paweh_tdt_pct(result$power[[scenario]], 1)),
      .paweh_summary_row("Affected-child trios", .paweh_tdt_count(result$N, plan = TRUE)),
      if (scenario != "no_error") .paweh_summary_row(
        "Absolute power loss",
    ...
    body <- shiny::tagList(
      rate_row,
      .paweh_summary_row("Required affected-child trios", .paweh_tdt_count(result$N[[scenario]], plan = TRUE)),
      .paweh_summary_row("Target power", .paweh_tdt_pct(result$target_power, 1)),
      if (scenario != "no_error") .paweh_summary_row(
        "Increase", if (is.finite(result$N[[scenario]] - baseline)) {
          paste0("+", .paweh_tdt_count(ceiling(result$N[[scenario]]) - ceiling(baseline), plan = TRUE), " trios")
        } else "not defined"
      ),
      if (scenario != "no_error") .paweh_summary_row(
        "Percent increase",
  ...
  bslib::card(
    ...
    bslib::card_header(unname(.paweh_tdt_scenario_labels[scenario])),
    bslib::card_body(shiny::div(class = "paweh-summary-grid", body))
  )
}

.paweh_tdt_interpretation <- function(calculation) {
  snapshot <- calculation$snapshot
  result <- calculation$result
  scenarios <- .paweh_tdt_scenarios(calculation)
  if (snapshot$objective == "power") {
    text <- paste0(
      "Under the specified assumptions, a study with ", .paweh_tdt_count(result$N, TRUE),
      " affected-child trios has an estimated no-error power of ",
      .paweh_tdt_pct(result$power$no_error, 1), "."
    )
    additions <- vapply(setdiff(scenarios, "no_error"), function(scenario) {
      paste0(" The ", tolower(.paweh_tdt_scenario_labels[[scenario]]),
        " scenario has estimated power of ", .paweh_tdt_pct(result$power[[scenario]], 1), ".")
    }, "")
  } else {
    text <- paste0(
      "Under the no-error model, approximately ", .paweh_tdt_count(result$N$no_error, TRUE),
...
.paweh_tdt_results_ui <- function(calculation) {
  scenarios <- .paweh_tdt_scenarios(calculation)
  shiny::tagList(
    shiny::div(class = "paweh-model-specification", .paweh_tdt_model_summary(calculation)),
    ...
    shiny::div(
      ...
      shiny::p(.paweh_tdt_interpretation(calculation))
    ),
    .paweh_tdt_advanced_ui(calculation)
  )
}

.paweh_tdt_specs <- function(calculation) {
  snapshot <- calculation$snapshot
...
.paweh_tdt_sensitivity <- function(calculation, parameter, range, n = 40L) {
  spec <- .paweh_tdt_specs(calculation)[[parameter]]
  ...
  data <- do.call(rbind, lapply(grid, function(value) {
    ...
    result <- tryCatch(.paweh_tdt_call(snapshot, args), error = function(e) NULL)
    ...
    data.frame(
      x = value, Scenario = unname(.paweh_tdt_scenario_labels[scenarios]),
...
.paweh_tdt_sensitivity_plot <- function(sensitivity) {
  ...
  ggplot2::ggplot(sensitivity$plot_data, ggplot2::aes(
    ...
    .paweh_plot_theme() + ggplot2::theme(legend.position = "top")
}

.paweh_tdt_transmissions <- function(calculation) {
  scenarios <- .paweh_tdt_scenarios(calculation)
  result <- calculation$result
  do.call(rbind, lapply(scenarios, function(scenario) data.frame(
    Scenario = unname(.paweh_tdt_scenario_labels[scenario]),
...
.paweh_tdt_transmission_plot <- function(calculation) {
  colors <- .paweh_plot_colors()
  ggplot2::ggplot(.paweh_tdt_transmissions(calculation), ggplot2::aes(
...
.paweh_tdt_summary_ui <- function(calculation) {
  snapshot <- calculation$snapshot
  values <- snapshot$display
  modifiers <- c(
  ...
  common <- list(
    ...
    .paweh_summary_row("Objective", if (snapshot$objective == "power") {
      paste("Estimate power with", .paweh_tdt_count(snapshot$objective_value, TRUE), "trios")
    } else paste("Minimum sample size at", .paweh_tdt_pct(snapshot$objective_value))),
    .paweh_summary_row("Inputs", if (snapshot$input_mode == "model_based") "Genetic model" else "Direct transmission quantities"),
    .paweh_summary_row("Alpha", format(values$alpha))
  )
  specific <- if (snapshot$input_mode == "model_based") list(
    .paweh_summary_row("Disease prevalence", paste0(values$prev, "%")),
    .paweh_summary_row("Allele frequency", paste0(values$pd, "%")),
    .paweh_summary_row("Relative risks", paste0("R1 ", values$R1, "; R2 ", values$R2)),
    .paweh_summary_row("D\u2032", format(values$delta_prime))
  ) else list(
    .paweh_summary_row("Transmission counts", paste0("ET ", values$ET, "; ENT ", values$ENT)),
    if (snapshot$objective == "mssn") .paweh_summary_row("Represented trios", .paweh_tdt_count(values$n_trios, TRUE))
  )
  shiny::div(
    class = "paweh-summary-grid", common, specific,
    .paweh_summary_row("Modifiers", if (length(modifiers)) paste(modifiers, collapse = "; ") else "None")
  )
}

.paweh_tdt_methods_ui <- function(calculation = NULL) {
  if (is.null(calculation)) return(.paweh_empty_ui("Methods"))
  scenarios <- unname(.paweh_tdt_scenario_labels[.paweh_tdt_scenarios(calculation)])
  shiny::tagList(
    shiny::h3("Methods"),
    shiny::div(
      class = "paweh-summary-grid",
      .paweh_summary_row("Study design", "Affected-child trio / TDT"),
      .paweh_summary_row("Objective", if (calculation$snapshot$objective == "power") "Power" else "Minimum sample size"),
      .paweh_summary_row("Input specification", if (calculation$snapshot$input_mode == "model_based") "Genetic model" else "Direct transmission quantities"),
      .paweh_summary_row("Statistical method", "Transmission Disequilibrium Test"),
...
.paweh_tdt_surface_axis_labels <- c(
...
.paweh_tdt_surface_scenarios <- function(calculation) {
  c(
    if (calculation$active$misclassification) "misclassification",
    if (calculation$active$heterogeneity) "heterogeneity",
    "no_error"
...
.paweh_tdt_surface_axes <- function(scenario) {
  c(
...
.paweh_tdt_surface_args <- function(calculation, scenario, x, y, n = 12L) {
  ...
  list(
    ...
    x = x, y = y, x_values = .paweh_tdt_surface_grid(x, n),
    y_values = .paweh_tdt_surface_grid(y, n),
...
.paweh_tdt_surface <- function(calculation, scenario, x, y, n = 12L) {
  plot <- do.call(
    plot_tdt_surface3d,
    .paweh_tdt_surface_args(calculation, scenario, x, y, n)
  )
  plotly::style(
    plot,
...
.paweh_tdt_server <- function(id) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    values <- shiny::reactive(.paweh_tdt_values(input))
    changed <- shiny::reactive(!is.null(state$signature) && !identical(state$signature, .paweh_tdt_sig(values())))
    ...
    shiny::observeEvent(input$calculate, {
      ...
      submitted <- values()
      tryCatch({
        state$calculation <- .paweh_tdt_calculate(.paweh_tdt_snapshot(submitted))
        state$signature <- .paweh_tdt_sig(submitted)
    ...
    } else .paweh_tdt_summary_ui(state$calculation))
    ...
    shiny::observeEvent(input$sensitivity_parameter, {
      ...
      spec <- .paweh_tdt_specs(state$calculation)[[input$sensitivity_parameter]]
      shiny::req(spec)
    ...
    output$sensitivity_plot <- shiny::renderPlot({
      ...
      .paweh_tdt_sensitivity_plot(state$sensitivity)
    })
    output$visualize_intro <- shiny::renderUI(if (is.null(state$calculation)) {
      .paweh_empty_ui("Visualize")
    } else shiny::div(
      class = "paweh-visual-intro", shiny::h3("Transmission imbalance"),
    ...
    output$transmission_plot <- shiny::renderPlot({
      shiny::req(state$calculation)
      .paweh_tdt_transmission_plot(state$calculation)
    })
    output$surface_controls <- shiny::renderUI({
      ...
      scenarios <- .paweh_tdt_surface_scenarios(state$calculation)
      ...
      shiny::div(
        class = "paweh-sensitivity-controls",
        shiny::selectInput(ns("surface_scenario"), "Scenario", stats::setNames(scenarios, .paweh_tdt_scenario_labels[scenarios]), selected = scenario),
        shiny::selectInput(ns("surface_x"), "X parameter", stats::setNames(axes, .paweh_tdt_surface_axis_labels[axes]), selected = x),
        shiny::selectInput(ns("surface_y"), "Y parameter", stats::setNames(setdiff(axes, x), .paweh_tdt_surface_axis_labels[setdiff(axes, x)]), selected = y),
    ...
    output$methods <- shiny::renderUI(.paweh_tdt_methods_ui(state$calculation))
