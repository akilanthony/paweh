# Getting started with pawh

## What problem does `pawh` solve?

`pawh` supports prospective power and minimum-sample-size-necessary
...
not estimates produced from raw genomic data by `pawh`.
...
For continuous traits, `pawh` supports one-way genotype-group ANOVA,
...
`pawh` is being prepared for Bioconductor because it addresses
...
- **Published-example reproduction:** a reported analytic result is
  evaluated with a matching `pawh` design.
...
Once `pawh` is available through Bioconductor, the standard installation
path will be:
